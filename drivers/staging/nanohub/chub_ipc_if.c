...
static void cipc_func_handle_irq(int evt, void *priv)
{
	...
	case IRQ_EVT_C2A_INT:
		if (atomic_read(&chub->atomic.irq_apInt) == C2A_OFF) {
			atomic_set(&chub->atomic.irq_apInt, C2A_ON);
...
int contexthub_get_sensortype(struct contexthub_ipc_info *chub, char *buf)
{
	...
	if (ipc_have_sensor_info(sensor_map)) {
		pack->num_os = os_image[chub->misc.num_os][os_name_idx] - '0';
		memcpy_fromio(&pack->sensormap, sensor_map, sizeof(struct sensor_map));
...
int contexthub_chub_ipc_init(struct contexthub_ipc_info *chub)
{
	...
	chub->chub_ipc =
		ipc_init(IPC_OWN_AP, IPC_SRC_MB0, chub->iomem.sram, chub->iomem.mailbox,
				&cipc_func);
	if (!chub->chub_ipc) {
	...
	ret =
		cipc_register(chub->iomem.mailbox, CIPC_USER_CHUB2AP,
...
int contexthub_ipc_drv_init(struct contexthub_ipc_info *chub)
{
	...
	if (ret) {
		...
		ret = chub_dbg_init(chub, chub->log.chub_rt_log.buffer,
				    chub->log.chub_rt_log.buffer_size);
		if (ret)
...
static bool contexthub_lowlevel_alive(struct contexthub_ipc_info *chub)
{
	...
	ipc_hw_gen_interrupt(chub->iomem.mailbox, chub_ipc->opp_mb_id,
			     IRQ_NUM_CHUB_ALIVE);
	ret = chub_wait_event(&chub->event.chub_alive_lock, 200);
	nanohub_dev_info(chub->dev, "%s done: ret:%d\n", __func__, ret);
	return atomic_read(&chub->event.chub_alive_lock.flag);
}
			       ...
			       enum mailbox_event event)
{
	...
	case MAILBOX_EVT_CHUB_ALIVE:
		...
		if (val) {
			atomic_set(&chub->atomic.chub_status, CHUB_ST_RUN);
			nanohub_dev_info(chub->dev, "%s : chub is alive\n", __func__);
		} else if (!chub->misc.multi_os) {
			nanohub_dev_err(chub->dev,
	...
	if (need_ipc) {
		...
		case MAILBOX_EVT_RT_LOGLEVEL:
			ipc_logbuf_loglevel(chub->log.chub_rt_log.loglevel, 1);
			break;
		...
		case MAILBOX_EVT_WAKEUP_CLR:
			if (atomic_read(&chub->atomic.wakeup_chub) == CHUB_ON) {
				ret =
				    cipc_add_evt(CIPC_REG_EVT_AP2CHUB,
						 IRQ_EVT_A2C_WAKEUP_CLR);
				if (ret >= 0) {
					atomic_set(&chub->atomic.wakeup_chub,
						   CHUB_OFF);
					clear_err_cnt(chub,
					...
					nanohub_dev_warn(chub->dev,
							 ...
							 __func__, ret,
							 chub->misc.err_cnt[CHUB_ERR_EVTQ_WAKEUP_CLR]);
					contexthub_handle_debug(chub, CHUB_ERR_EVTQ_WAKEUP_CLR);
				}
			}
			break;
		case MAILBOX_EVT_WAKEUP:
			if (atomic_read(&chub->atomic.wakeup_chub) == CHUB_OFF) {
				ret =
				    cipc_add_evt(CIPC_REG_EVT_AP2CHUB,
						 IRQ_EVT_A2C_WAKEUP);
				if (ret >= 0) {
					atomic_set(&chub->atomic.wakeup_chub, CHUB_ON);
					clear_err_cnt(chub, CHUB_ERR_EVTQ_WAKEUP);
				} else {
					nanohub_dev_warn(chub->dev,
							 "%s: fails to set wakeupclr. ret:%d, err:%d",
							 __func__, ret,
							 chub->misc.err_cnt[CHUB_ERR_EVTQ_WAKEUP]);
		...
		if (ret < 0) {
			...
			nanohub_dev_warn(chub->dev,
					 ...
					 __func__,
					 chub->misc.err_cnt[CHUB_ERR_EVTQ_ADD],
					 chub->misc.err_cnt[CHUB_ERR_EVTQ_WAKEUP],
					 chub->misc.err_cnt[CHUB_ERR_EVTQ_WAKEUP_CLR]);
		} else {
			 ...
			 uint8_t *tx, int length, int timeout)
{
	...
	if (__raw_readl(&chub->atomic.chub_status) != CHUB_ST_RUN) {
		nanohub_dev_warn(chub->dev, "%s: chub isn't run:%d\n",
				 __func__, __raw_readl(&chub->atomic.chub_status));
		contexthub_handle_debug(chub, CHUB_ERR_CHUB_ST_ERR);
		return 0;
	}
	clear_err_cnt(chub, CHUB_ERR_CHUB_ST_ERR);
	...
	if (ret) {
		nanohub_err
		    ("%s: fails to write data: ret:%d, len:%d errcnt:%d\n",
		     __func__, ret, length, chub->misc.err_cnt[CHUB_ERR_WRITE_FAIL]);
			...
			int max_length, int timeout)
{
	...
	if (atomic_read(&chub->read_lock.cnt)) {
		...
		if (rxbuf != NULL && size > 0) {
			...
			nanohub_dev_info(chub->dev, "get abnormal buffer.\n");
			nanohub_dev_info(chub->dev, "rxbuf : %p, size : %d\n", rxbuf, size);
			nanohub_dev_info(chub->dev, "skip this\n");
		}

		atomic_dec(&chub->read_lock.cnt);
	} else {
		if (chub->misc.err_cnt[CHUB_ERR_READ_FAIL])
			nanohub_dev_info(chub->dev,
					 "%s: read timeout(%d): c2aq_cnt:%d, recv_cnt:%d during %lld ns\n",
					 __func__,
					 chub->misc.err_cnt[CHUB_ERR_READ_FAIL],
		...
		if (chub->misc.err_cnt[CHUB_ERR_READ_FAIL]) {
			nanohub_err
			    ("%s: fails to read data: ret:%d, len:%d errcnt:%d, err:%d\n",
			     __func__, ret, chub->misc.err_cnt[CHUB_ERR_READ_FAIL],
			     restart_err);
			ipc_dump();
			if (!restart_err)
...
int contexthub_ipc_if_init(struct contexthub_ipc_info *chub)
{
	...
	chub->data->irq = chub->irq.irq_mailbox;
#endif
