...
int slsi_mlme_set(struct slsi_dev *sdev, struct net_device *dev, u8 *mib, int mib_len)
{
	...
	cfm = slsi_mlme_set_with_cfm(sdev, dev, mib, mib_len);
	if (!cfm)
		return -EIO;
...
int slsi_mlme_start(struct slsi_dev *sdev, struct net_device *dev, u8 *bssid, struct cfg80211_ap_settings *settings, const u8 *wpa_ie_pos, const u8 *wmm_ie_pos, bool append_vht_ies)
{
	...
	req = fapi_alloc(mlme_start_req, MLME_START_REQ, ndev_vif->ifnum, total_len);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 41))
			bss_color_ie[SLSI_WLAN_HE_BSS_COLOR_ENABLE_DISABLE_INDEX] = (settings->beacon.he_bss_color.partial << SLSI_WLAN_HE_BSS_COLOR_PARTIAL_OFFSET) |
...
void slsi_mlme_connect_resp(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	SLSI_NET_INFO(dev, "mlme_connect_resp(vif:%u)\n", ndev_vif->ifnum);
...
void slsi_mlme_connected_resp(struct slsi_dev *sdev, struct net_device *dev, u16 peer_index)
{
	...
	flow_id = ((peer_index & 0xFF) << 8) | (0xFF);
			   ...
			   int rssi, int rssi_th_2g, int rssi_th_5g, int rssi_th_6g)
{
	...
	req = fapi_alloc(mlme_set_wtc_mode_req, MLME_SET_WTC_MODE_REQ, ndev_vif->ifnum, 0);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}
...
int slsi_mlme_set_country(struct slsi_dev *sdev, char *alpha2)
{
	...
	SLSI_DBG2(sdev, SLSI_MLME, "mlme_set_country_req(country:%c%c, operating_class_set:%x)\n", alpha2[0], alpha2[1], operating_class_set);
...
int slsi_mlme_configure_monitor_mode(struct slsi_dev *sdev, struct net_device *dev, struct cfg80211_chan_def *chandef)
{
	...
	req = fapi_alloc(mlme_configure_monitor_mode_req, MLME_CONFIGURE_MONITOR_MODE_REQ, ndev_vif->ifnum, 0);
	if (!req) {
...
int slsi_mlme_twt_setup(struct slsi_dev *sdev, struct net_device *dev, struct twt_setup *tsetup)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_twt_setup_req, MLME_TWT_SETUP_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
...
int slsi_mlme_twt_teardown(struct slsi_dev *sdev, struct net_device *dev, u16 setup_id, u16 negotiation_type)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_twt_teardown_req, MLME_TWT_TEARDOWN_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
...
int slsi_mlme_twt_suspend(struct slsi_dev *sdev, struct net_device *dev, u16 setup_id, u32 suspend_duration)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_twt_suspend_req, MLME_TWT_SUSPEND_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;

	fapi_set_u16(req, u.mlme_twt_suspend_req.twt_setup_id, setup_id);
	fapi_set_u32(req, u.mlme_twt_suspend_req.twt_suspend_duration, suspend_duration);

	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_TWT_SUSPEND_CFM);
	if (!cfm)
		return -EIO;

	if (fapi_get_u16(cfm, u.mlme_twt_suspend_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_NET_ERR(dev, "mlme_twt_suspend_cfm(result:0x%04x) ERROR\n",
			     fapi_get_u16(cfm, u.mlme_twt_suspend_cfm.result_code));
		r = -EINVAL;
	}
	kfree_skb(cfm);
	return r;
			       ...
			       int buf_len, u16 setup_id)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_twt_status_query_req, MLME_TWT_STATUS_QUERY_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
	...
	r = snprintf(command, buf_len, "%d %d %d %d\n", setup_id,
...
int slsi_mlme_set_adps_state(struct slsi_dev *sdev, struct net_device *dev, int adps_state)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_set_adps_state_req, MLME_SET_ADPS_STATE_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
			     ...
			     struct slsi_scheduled_pm_setup *sched_pm_setup)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_scheduled_pm_setup_req, MLME_SCHEDULED_PM_SETUP_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
	...
	r = fapi_get_u16(cfm, u.mlme_scheduled_pm_setup_cfm.result_code);
...
int slsi_mlme_sched_pm_teardown(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_scheduled_pm_teardown_req, MLME_SCHEDULED_PM_TEARDOWN_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
	...
	if (!cfm)
		return -EIO;
	r = fapi_get_u16(cfm, u.mlme_scheduled_pm_teardown_cfm.result_code);
	if (r != FAPI_RESULTCODE_SUCCESS)
...
int slsi_mlme_sched_pm_status(struct slsi_dev *sdev, struct net_device *dev, struct slsi_scheduled_pm_status *status)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_scheduled_pm_get_status_req, MLME_SCHEDULED_PM_GET_STATUS_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
...
int slsi_mlme_set_delayed_wakeup(struct slsi_dev *sdev, struct net_device *dev, int delayed_wakeup_on, int timeout)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_delayed_wakeup_req, MLME_DELAYED_WAKEUP_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
				      ...
				      int ipv4_count, u8 ipv6addrlist[][16], int ipv6_count)
{
	...
	req = fapi_alloc(mlme_delayed_wakeup_req, MLME_DELAYED_WAKEUP_REQ, ndev_vif->ifnum, alloc_data_size);
	if (!req)
		return -ENOMEM;
...
int slsi_mlme_ap_suspend(struct slsi_dev *sdev, struct net_device *dev, u16 suspend_mode)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_set_ap_suspend_state_req, MLME_SET_AP_SUSPEND_STATE_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
				...
				struct ap_rps_params *rps_params, u16 ifnum)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_ap_rps_params_req, MLME_AP_RPS_PARAMS_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;

	fapi_set_u16(req, u.mlme_ap_rps_params_req.vif, ifnum);
...
int slsi_mlme_set_ap_rps(struct slsi_dev *sdev, struct net_device *dev, u16 rps_enable, u16 ifnum)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_ap_rps_req, MLME_AP_RPS_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;

	fapi_set_u16(req, u.mlme_ap_rps_req.vif, ifnum);
...
int slsi_mlme_set_ap_tx_power(struct slsi_dev *sdev, struct net_device *dev, u16 tx_power, u16 ifnum)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_set_ap_tx_power_req, MLME_SET_AP_TX_POWER_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;

	fapi_set_u16(req, u.mlme_set_ap_tx_power_req.vif, ifnum);
			      ...
			      int buf_len, u16 ifnum)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_get_ap_tx_power_req, MLME_GET_AP_TX_POWER_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;

	fapi_set_u16(req, u.mlme_get_ap_tx_power_req.vif, ifnum);
...
int slsi_mlme_scheduled_pm_leaky_ap_detect(struct slsi_dev *sdev, struct net_device *dev, struct slsi_sched_pm_leaky_ap *leaky_ap)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_scheduled_pm_leaky_ap_detect_req, MLME_SCHEDULED_PM_LEAKY_AP_DETECT_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
