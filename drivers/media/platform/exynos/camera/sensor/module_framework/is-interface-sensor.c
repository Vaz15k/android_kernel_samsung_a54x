		...
		u32 *exposure_count)
{
	...
	if (sensor->cfg->output[CSI_VIRTUAL_CH_0].hwformat == HW_FORMAT_RAW12) {
		...
		camera2_udm_t *udm)
{
	...
	dbg_sensor(1, "[%s] (F:%d): udm_dgain(L(%d), S(%d), M(%d))\n",
		__func__, frame_count,
		udm->sensor.longDigitalGain,
		udm->sensor.shortDigitalGain,
		udm->sensor.midDigitalGain);
	dbg_sensor(1, "[%s] (F:%d): udm_again(L(%d), S(%d), M(%d))\n",
		__func__, frame_count,
	...
	u32 *dgain)
{
	...
	if (!itf) {
		err("[%s] is_sensor_interface is NULL", __func__);
		return -EINVAL;
	}
...
struct is_subdev *get_vc_subdev(struct is_sensor_interface *itf,
		enum itf_vc_buf_data_type request_data_type)
{
	...
	if (!ischain) {
		err("failed to get ischain device");
		return ERR_PTR(-ENODEV);
	}
	...
	case VC_BUF_DATA_TYPE_GENERAL_STAT1:
		vc_type = VC_PRIVATE;
		subdev = &ischain->pdst; /* This is supported after PDP v1.0.0 */
		break;
	...
	if (!subdev || !test_bit(IS_SUBDEV_START, &subdev->state)) {
		for (ch = CSI_VIRTUAL_CH_1; ch < CSI_VIRTUAL_CH_MAX; ch++) {
			if (sensor->cfg->output[ch].type == vc_type)
				break;
		}
...
static int update_pd_vc_buf_masking(struct v4l2_subdev *v4l2_subdev,
	struct is_subdev *subdev, int idx, bool use)
{
	int ret;
	...
	args.vc_dma = subdev;
	args.start_end = use;
	args.csi = (struct is_device_csi *)v4l2_get_subdevdata(v4l2_subdev);

	ret = v4l2_subdev_call(v4l2_subdev, core, ioctl, SENSOR_IOCTL_CSI_S_PDBUF_MASKING, &args);
		...
		u64 *buf_addr)
{
	...
	*buf_index = 0;
	...
	subdev = get_vc_subdev(itf, request_data_type);
	if (IS_ERR_OR_NULL(subdev)) {
		err("failed to get_vc_subdev");
		return -ENODEV;
	}

	sensor = get_device_sensor(itf);
	if (!sensor) {
		merr("failed to get sensor device", subdev);
		return -ENODEV;
	...
	if (request_data_type == VC_BUF_DATA_TYPE_SENSOR_STAT1) {
		if (!sensor->subdev_csi) {
			err("Sensor sub_csi is null");
			return -EINVAL;
		}

		update_pd_vc_buf_masking(sensor->subdev_csi, subdev, *buf_index, 1);
	}

	dbg_sensor(2, "[%s]: [%d][%s] index: %d, framecount: %d, addr: 0x%llx\n",
		__func__, subdev->instance, subdev->name, *buf_index, frame_count, *buf_addr);
		...
		u32 frame_count)
{
	...
	dbg_sensor(1, "[%s]: [%d][%s] index: %d\n", __func__, subdev->instance, subdev->name, frame->index);
	...
	if (request_data_type == VC_BUF_DATA_TYPE_SENSOR_STAT1) {
		...
		update_pd_vc_buf_masking(sensor->subdev_csi, subdev, frame->index, 0);
	}
	return 0;
		...
		u32 index)
{
	...
	if (index >= framemgr->num_frames) {
		mserr("index(%d of %d) is out-of-range", subdev, subdev, index, framemgr->num_frames);
		return -ENOENT;
	}
	...
	dbg_sensor(1, "[%s]: [%d][%s] index: %d\n", __func__, subdev->instance, subdev->name, index);
	...
	if (request_data_type == VC_BUF_DATA_TYPE_SENSOR_STAT1) {
		...
		update_pd_vc_buf_masking(sensor->subdev_csi, subdev, index, 0);
	}
	return 0;
}
		...
		struct vc_buf_info_t *buf_info)
{
	...
	struct v4l2_subdev *subdev_module;
	struct is_device_sensor *sensor;
	u32 ch = CSI_VIRTUAL_CH_MAX, stat_vc;
	u32 dummy_pixel = 0;
	...
	buf_info->width = sensor->cfg->input[stat_vc].width + dummy_pixel;
	buf_info->height = sensor->cfg->input[stat_vc].height;
...
int init_sensor_interface(struct is_sensor_interface *itf)
{
	...
	itf->cis_itf_ops.update_latest_seamless_state = update_latest_seamless_state;
