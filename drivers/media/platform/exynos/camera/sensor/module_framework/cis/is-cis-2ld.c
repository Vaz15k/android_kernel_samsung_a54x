...
int sensor_2ld_cis_set_aeb_mode_change(struct v4l2_subdev *subdev)
{
	...
	case SENSOR_HDR_MODE_2AEB_2VC:
		pr_info("%s : enable 2AEB_2VC\n", __func__);
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write8(cis->client, 0x0E0B, 0x04);
		break;
	case SENSOR_HDR_MODE_2AEB_1VC:
		pr_info("%s : enable 2AEB 1VC\n", __func__);
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write8(cis->client, 0x0E0B, 0x04);
		ret |= cis->ixc_ops->write16(cis->client, 0x0E0C, 0x0300);
		ret |= cis->ixc_ops->write16(cis->client, 0x0E0E, 0x0000); // all register 6*2 offset
		ret |= cis->ixc_ops->write16(cis->client,0x0B30, 0x0000); // LN OFF
		break;
	case SENSOR_HDR_MODE_SINGLE:
		if (!sensor_2ld_LTE_1s_flag) {
			pr_info("%s : disable AEB\n", __func__);
			ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
			ret |= cis->ixc_ops->write8(cis->client, 0x0E0B, 0x00);
		}
		break;
	default:
		dbg_sensor(1, "[%s] not support aeb mode(%d)\n",
				__func__, cis->cis_data->cur_hdr_mode);
	}
...
static const struct is_cis_log log_2ld[] = {
	/* 4000 page */
	...
	{I2C_READ, 16, 0x0230, 0, "0x0230"},
	{I2C_READ, 16, 0x0B30, 0, "0x0B30"},
	{I2C_READ, 16, 0x0A7A, 0, "0x0A7A"},
	{I2C_READ, 16, 0x0A7C, 0, "0x0A7C"},
	{I2C_READ, 8 , 0x0E0B, 0, "0x0E0B"},
	/* 2000 page */
	{I2C_WRITE, 16, 0x602C, 0x2000, "0x2000 page"},
	{I2C_WRITE, 16, 0x602E, 0xD570, NULL},
	{I2C_READ, 16, 0x6F12, 0, "0xD570"},
	/* 2007 page */
	{I2C_WRITE, 16, 0x602C, 0x2007, "0x2007 page"},
	{I2C_WRITE, 16, 0x602E, 0xF4B2, NULL},
	{I2C_READ, 16, 0x6F12, 0, "0xF4B2"},
	{I2C_WRITE, 16, 0x602E, 0xF4B6, NULL},
	{I2C_READ, 16, 0x6F12, 0, "0xF4B6"},
	{I2C_WRITE, 16, 0x602E, 0xF4B8, NULL},
	{I2C_READ, 16, 0x6F12, 0, "0xF4B8"},
	/* restore 4000 page */
	{I2C_WRITE, 16, 0xFCFC, 0x4000, NULL},
	{I2C_WRITE, 16, 0x602C, 0x4000, "restore to 0x4000 page"},
};
...
int sensor_2ld_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	sensor_cis_log_status(cis, log_2ld,
			ARRAY_SIZE(log_2ld), (char *)__func__);
...
int sensor_2ld_cis_set_global_setting_internal(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u16 rev = 0;
	bool otp_module = true;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_2ld_private_data *priv = (struct sensor_2ld_private_data *)cis->sensor_info->priv;

	rev = cis->cis_data->cis_rev;

	info("[%s] global setting start (rev =%x)\n", __func__, rev);
	...
	if (otp_module)
		ret = sensor_cis_write_registers_locked(subdev, priv->global_a3);
	else 
		ret = sensor_cis_write_registers_locked(subdev, priv->global_a2);
...
int sensor_2ld_cis_set_global_setting_retention(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_cis_write_registers_locked(subdev, priv->global_retention);
	if (ret < 0) {
		err("sensor_2ld_set_registers fail!!");
		goto p_err;
	}
...
int sensor_2ld_cis_retention_crc_enable(struct v4l2_subdev *subdev, u32 mode)
{
	...
	default:
		/* Sensor stream on */
		cis->ixc_ops->write16(client, 0x0100, 0x0100);
...
int sensor_2ld_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (ext_info->use_retention_mode == SENSOR_RETENTION_ACTIVATED) {
		if (sensor_2ld_load_retention == false) {
			...
			cis->ixc_ops->write16(cis->client, 0x602A, 0x0E1C);
		...
		case SENSOR_2LD_4032X3024_30FPS:
			...
			ret = sensor_cis_write_registers(subdev, priv->load_sram[SENSOR_2LD_4032x3024_30FPS_LOAD_SRAM]);
			if (ret < 0) {
				err("sensor_2ld_set_registers fail!!");
		...
		case SENSOR_2LD_4032X2268_30FPS:
			...
			ret = sensor_cis_write_registers(subdev, priv->load_sram[SENSOR_2LD_4032x2268_30FPS_LOAD_SRAM]);
			if (ret < 0) {
				err("sensor_2ld_set_registers fail!!");
				goto p_err_i2c_unlock;
			}
		...
		case SENSOR_2LD_4032X2268_60FPS:
			...
			ret = sensor_cis_write_registers(subdev, priv->load_sram[SENSOR_2LD_4032x2268_60FPS_LOAD_SRAM]);
			if (ret < 0) {
				err("sensor_2ld_set_registers fail!!");
				goto p_err_i2c_unlock;
			}
		...
		case SENSOR_2LD_1008X756_120FPS_MODE2:
			...
			ret = sensor_cis_write_registers(subdev, priv->load_sram[SENSOR_2LD_1008x756_120FPS_LOAD_SRAM]);
			if (ret < 0) {
				err("sensor_2ld_set_registers fail!!");
				goto p_err_i2c_unlock;
			}
		...
		case SENSOR_2LD_4032X3024_24FPS:
			...
			ret = sensor_cis_write_registers(subdev, priv->load_sram[SENSOR_2LD_4032x3024_24FPS_LOAD_SRAM]);
			if (ret < 0) {
				err("sensor_2ld_set_registers fail!!");
				goto p_err_i2c_unlock;
			}
		...
		case SENSOR_2LD_4032X2268_24FPS:
			...
			ret = sensor_cis_write_registers(subdev, priv->load_sram[SENSOR_2LD_4032x2268_24FPS_LOAD_SRAM]);
			if (ret < 0) {
				err("sensor_2ld_set_registers fail!!");
				goto p_err_i2c_unlock;
			}
	...
	if (mode == SENSOR_2LD_2016X1134_60FPS_MODE2_SSM_960
		...
		ret |= cis->ixc_ops->write8(cis->client, 0x0120, 0x01);

		ret |= cis->ixc_ops->read8(cis->client, 0x1982, &data8);
		if (unlikely(!ret))
			pr_info("0x1982(0x%x)\n", data8);
		else
			goto p_err_i2c_unlock;
...
int sensor_2ld_cis_set_lownoise_mode_change(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->read8(cis->client, 0x0005, &sensor_2ld_ln_mode_frame_count);
	...
	case IS_CIS_LNOFF:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write16(cis->client, 0x0B30, 0x0100);
#ifdef CAMERA_REAR2
		switch (mode) {
		case SENSOR_2LD_4032X3024_60FPS:
		case SENSOR_2LD_4032X3024_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x2A30);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X2268_60FPS:
		case SENSOR_2LD_4032X2268_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x2B70);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X3024_24FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x3606);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X2268_24FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x3740);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		}
#endif
	...
	case IS_CIS_LN2:
		...
		case SENSOR_2LD_4032X3024_60FPS:
		case SENSOR_2LD_4032X3024_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0065);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X2268_60FPS:
		case SENSOR_2LD_4032X2268_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0060);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X3024_24FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0066);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X2268_24FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0025);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		}
#endif
		...
		cis->cis_data->line_readOut_time = cis->cis_data->line_readOut_time * 2;
		cis->cis_data->frame_valid_us_time = cis->cis_data->frame_valid_us_time * 2;
		cis->cis_data->frame_time = (cis->cis_data->line_readOut_time * cis->cis_data->cur_height / 1000);
		break;
	case IS_CIS_LN4:
		dbg_sensor(1, "[%s] IS_CIS_LN4\n", __func__);
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write16(cis->client, 0x0B30, 0x0102);
#ifdef CAMERA_REAR2
		switch (mode) {
		case SENSOR_2LD_4032X3024_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0265);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X2268_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x01F0);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X3024_24FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0240);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		case SENSOR_2LD_4032X2268_24FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x01C0);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x0010);
			break;
		default:
		...
		cis->cis_data->rolling_shutter_skew = (cis->cis_data->cur_height - 1) * cis->cis_data->line_readOut_time * 3;
		cis->cis_data->line_readOut_time = cis->cis_data->line_readOut_time * 3;
		cis->cis_data->frame_valid_us_time = cis->cis_data->frame_valid_us_time * 3;
		cis->cis_data->frame_time = (cis->cis_data->line_readOut_time * cis->cis_data->cur_height / 1000);
		break;
...
int sensor_2ld_cis_retention_prepare(struct v4l2_subdev *subdev)
{
	...
	ret |= cis->ixc_ops->write16(cis->client, 0x6028, 0x3000);
	ret |= cis->ixc_ops->write16(cis->client, 0x602A, 0x0484);
	ret |= cis->ixc_ops->write16(cis->client, 0x6F12, 0x0100);
	ret |= cis->ixc_ops->write16(cis->client, 0x010E, 0x0100);

	ret |= cis->ixc_ops->write16(cis->client, 0x0BCC, 0x0000); // Block mipi signal
...
int sensor_2ld_cis_retention_crc_check(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->read8(cis->client, 0x100E, &crc_check); /* api_ro_checksum_on_ram_passed */
...
int sensor_2ld_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (ex_mode == EX_DUALFPS_480 || ex_mode == EX_DUALFPS_960) {
		...
		cis->ixc_ops->write16(client, 0x6000, 0x0005);
		cis->ixc_ops->write16(client, 0xFCFC, 0x2000);
		cis->ixc_ops->write16(client, 0x8A10, 0x0009);
		cis->ixc_ops->write16(client, 0x8A0C, 0x03F7);
		cis->ixc_ops->write16(client, 0xFCFC, 0x4000);
	...
	/* Sensor stream on */
	info("%s\n", __func__);
	cis->ixc_ops->write16(client, 0x0BCC, 0x0100); // Enable mipi signal
...
int sensor_2ld_cis_wait_seamless_update_delay(struct v4l2_subdev *subdev)
{
...
#if IS_ENABLED(USE_CAMERA_SENSOR_RETENTION)
	/* retention mode CRC check register enable */
	cis->ixc_ops->write16(cis->client, 0x6028, 0x3000);
	cis->ixc_ops->write16(cis->client, 0x602A, 0x0484);
	cis->ixc_ops->write16(cis->client, 0x6F12, 0x0100);
...
int sensor_2ld_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	min_cit = cis_data->min_coarse_integration_time;
	...
	if (long_coarse_int < min_cit) {
		dbg_sensor(1, "[MOD:D:%d] %s, vsync_cnt(%d), long coarse(%d) min_cit(%d)\n", cis->id, __func__,
			cis_data->sen_vsync_count, long_coarse_int, min_cit);
		long_coarse_int = min_cit;
	}

	if (short_coarse_int < min_cit) {
		dbg_sensor(1, "[MOD:D:%d] %s, vsync_cnt(%d), short coarse(%d) min_cit(%d)\n", cis->id, __func__,
			cis_data->sen_vsync_count, short_coarse_int, min_cit);
		short_coarse_int = min_cit;
	}
	...
	/* AEB mode */
	if (cis->cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_1VC) {
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_CIT, long_coarse_int); // #1 short CIT
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_LCIT, long_coarse_int); // #1 long CIT
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_CIT, short_coarse_int); // #2 short CIT
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_LCIT, short_coarse_int); // #2 long CIT
		}
			if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_CIT, long_coarse_int); // #3 short CIT
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_LCIT, long_coarse_int); // #3 long CIT
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_CIT, short_coarse_int); // #4 short CIT
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_LCIT, short_coarse_int); // #4 long CIT
		}
		...
		if (sensor_2ld_cis_is_wdr_mode_on(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, 0x021E, 0x0100);
		} else {
			ret |= cis->ixc_ops->write16(cis->client, 0x021E, 0x0000);
		}

		if (sensor_2ld_LTE_1s_flag) {
			ret |= cis->ixc_ops->write16(cis->client, 0x0E1A, short_coarse_int);
			ret |= cis->ixc_ops->write16(cis->client, 0x0E1E, long_coarse_int);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
		...
		/* Long exposure */
		if (sensor_2ld_cis_is_wdr_mode_on(cis_data)) {
			ret |= cis->ixc_ops->write16(client, 0x0226, long_coarse_int);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
...
int sensor_2ld_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	if (cis->cis_data->sens_config_index_cur == SENSOR_2LD_1280X720_60FPS_MODE2_SSM_960) {
		if ((cis->cis_data->sen_vsync_count % 120) == 119) {
			...
			case 0:
				...
				cis->ixc_ops->write16(cis->client, 0x0A52, 0x0103);
				record_status++;
				break;
			case 5:
				info("%s - ssm_end_record : sen_vsync_count(%d)", __func__, cis->cis_data->sen_vsync_count);
				cis->ixc_ops->write8(cis->client, 0x0A51, 0x01);
				record_status++;
				break;
			case 10:
				record_status = 0;
	...
	if (cis->cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_1VC) {
		...
		frame_length_lines = (u16)((vt_pic_clk_freq_khz * frame_duration) / (line_length_pck * 1000));
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #1 FLL
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #2 FLL
		}
		if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #3 FLL
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #4 FLL
		}
		...
		if (sensor_2ld_LTE_1s_flag) {
			ret |= cis->ixc_ops->write16(client, 0x0E22, frame_length_lines);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
...
int sensor_2ld_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	if (cis->cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_1VC) {
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #1 Again
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #2 Again

		}
		if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #3 Again
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #4 Again
		}
		...
		if (sensor_2ld_LTE_1s_flag) {
			ret |= cis->ixc_ops->write16(client, 0x0E1C, analog_gain);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
...
int sensor_2ld_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (cis->cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_1VC) {
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_DGAIN, long_gain); // #1 Dgain
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_LDGAIN, long_gain); // #1 long Dgain

			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_DGAIN, short_gain); // #2 Dgain
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_LDGAIN, short_gain); // #2 long Dgain
		}
		if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_DGAIN, long_gain); // #3 Dgain
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_LDGAIN, long_gain); // #3 long Dgain

			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_DGAIN, short_gain); // #4 Dgain
			ret |= cis->ixc_ops->write16(client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_LDGAIN, short_gain); // #4 long Dgain
		}
		...
		if (sensor_2ld_LTE_1s_flag) {
			ret |= cis->ixc_ops->write16(client, 0x0E20, short_gain);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
		...
		/* Long digital gain */
		if (sensor_2ld_cis_is_wdr_mode_on(cis_data)) {
			ret |= cis->ixc_ops->write16(client, 0x0230, long_gain);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
...
int sensor_2ld_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	...
	if (lte_mode->sen_strm_off_on_enable) {
		...
		if (lte_mode->expo[0] >= 1000000) {
			...
			ret |= cis->ixc_ops->write16(cis->client, 0xBCA2, (shift_count << 8) | shift_count); // LTE FLL Shifter (1byte), CIT Shifter (1byte)
			ret |= cis->ixc_ops->write16(cis->client, 0xBCA4, (shift_count << 8) | 0x9E); // LTE Long CIT Shifter (1byte)
...
int sensor_2ld_cis_compensate_gain_for_extremely_br(struct v4l2_subdev *subdev, u32 expo, u32 *again, u32 *dgain)
{
	...
	if (coarse_int < min_cit) {
		dbg_sensor(1, "[MOD:D:%d] %s, vsync_cnt(%d), long coarse(%d) min_cit(%d)\n", cis->id, __func__,
			cis_data->sen_vsync_count, coarse_int, min_cit);
		coarse_int = min_cit;
	}
...
static struct is_cis_ops cis_ops_2ld = {
	...
	.cis_wait_seamless_update_delay = sensor_2ld_cis_wait_seamless_update_delay,
};

static int cis_2ld_probe_i2c(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	cis->sensor_info = &sensor_2ld_info_A;
