...
int is_sensor_peri_notify_vsync(struct v4l2_subdev *subdev, void *arg)
{
	...
	if (cis->use_seamless_mode)
		sensor_peri->sensor_interface.cis_itf_ops
			.update_latest_seamless_state(&sensor_peri->sensor_interface);
	...
	if (cis->use_notify_vsync)
		schedule_work(&cis->notify_vsync_work);
...
void is_sensor_cis_notify_vsync_work(struct work_struct *data)
{
	int ret;
	struct is_cis *cis;
	struct is_device_sensor_peri *sensor_peri;
	...
	cis = container_of(data, struct is_cis, notify_vsync_work);
	sensor_peri = container_of(cis, struct is_device_sensor_peri, cis);

	if (sensor_peri->subdev_cis) {
		ret = CALL_CISOPS(cis, cis_notify_vsync, cis->subdev);
		if (ret < 0) {
			err("err!!! cis_notify_vsync ret(%d)", ret);
			return;
		}
	}
}
...
void is_sensor_peri_init_work(struct is_device_sensor_peri *sensor_peri)
{
	...
	sensor_peri->cis.use_notify_vsync = false;
	INIT_WORK(&sensor_peri->cis.notify_vsync_work, is_sensor_cis_notify_vsync_work);
}
...
#ifndef USE_CAMERA_MCD_SW_DUAL_SYNC
			...
			if (sensor_peri->laser_af && test_bit(IS_SENSOR_LASER_AF_AVAILABLE, &sensor_peri->peri_state)) {
				if (sensor_peri->laser_af->rs_mode != CAMERA_RANGE_SENSOR_MODE_OFF) {
					if ((device->ischain->setfile & IS_SETFILE_MASK) != ISS_SUB_SCENARIO_VIDEO_SUPER_STEADY
						...
						info("[%s] supersteady, LAF not active\n", __func__);
					}
				} else {
					info("[%s] LAF CAMERA_RANGE_SENSOR_MODE_OFF\n", __func__);
				}
			}
#endif
		}
...
{
	struct is_device_sensor_peri *sensor_peri = get_sensor_peri(device->subdev_module);

	if (sensor_peri)
		return CALL_CISOPS(&sensor_peri->cis, cis_group_param_hold,
						sensor_peri->subdev_cis, hold);
...
void is_sensor_peri_compensate_duration_for_frame_sync(struct is_device_sensor *device, u32 vsync_count)
{
	...
	if (!cis_master) {
		dbg_sensor(2, "[%s]cis_master for sw frame sync is NULL", __func__);
		return;
	}
...
#ifdef USE_CAMERA_MCD_SW_DUAL_SYNC
	if (sensor_uctrl->dynamicFrameDuration > SW_SYNC_24FPS_DURATION) {
		add_criteria = (sensor_uctrl->dynamicFrameDuration - SW_SYNC_24FPS_DURATION)
						* (SW_SYNC_ADJUST_MAX_CRITERIA - SW_SYNC_ADJUST_CRITERIA)
						/ (SW_SYNC_10FPS_DURATION - SW_SYNC_24FPS_DURATION);
		if (add_criteria > (SW_SYNC_ADJUST_MAX_CRITERIA - SW_SYNC_ADJUST_CRITERIA))
			add_criteria = (SW_SYNC_ADJUST_MAX_CRITERIA - SW_SYNC_ADJUST_CRITERIA);

		dbg_sensor(2, "[%s]add_criteria(%lld)", __func__, add_criteria);
	}
#endif
	...
	dbg_sensor(2, "[%s][F:%d,%d,%d] T_stmp(%lld,%lld,%lld), Duration(%lld,%lld,%lld),(diff:%lld)", __func__,
...
#ifdef USE_CAMERA_MCD_SW_DUAL_SYNC
	if (pre_duration_m == 0 || cur_duration_m == 0 || cur_duration_m > SW_SYNC_MAX_DURATION) {
		dbg_sensor(2, "[%s]skip SW SYNC", __func__);
		return;
	}

	if ((cur_ts_s >= cur_ts_m) && ((cur_ts_s - cur_ts_m) >= SW_SYNC_ADJUST_CRITERIA + add_criteria)) {
		if ((cur_duration_m * 2) < (cur_ts_s - cur_ts_m)) {
			dbg_sensor(2, "[%s](exceed value cur_durationx2(%lld) < diff(%lld) skip SW SYNC", __func__,
				(cur_duration_m * 2), (cur_ts_s - cur_ts_m));
			return;
		}
		compensate_duration = (cur_duration_m * 2) - (cur_ts_s - cur_ts_m);
	} else if ((cur_ts_s < cur_ts_m) && ((cur_ts_m - cur_ts_s) >= SW_SYNC_ADJUST_CRITERIA + add_criteria)) {
		compensate_duration = pre_duration_m + (cur_ts_m - cur_ts_s);
	} else {
		dbg_sensor(2, "[%s][F:%d] S/W frame sync is working(diff:%lld)", __func__, vsync_count,
	...
	dbg_sensor(2, "[%s][F:%d] Duration is adjusted(%lld) for frame sync cur_duration_m(%lld)", __func__, vsync_count,
