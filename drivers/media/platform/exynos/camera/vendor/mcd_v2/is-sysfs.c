...
static struct device *camera_ois_dev;
#endif
...
static int ois_power_count = 0;
static int shaking_power_count = 0;
static int ois_threshold = 150;
#ifdef CAMERA_2ND_OIS
static int ois_threshold_rear2 = 150;
#endif
static long x_init_raw = 0;
static long y_init_raw = 0;
static long z_init_raw = 0;
#endif
...
static ssize_t camera_camfw_show(char *buf, enum is_cam_info_index cam_index, bool camfw_full)
{
	...
	if (!test_bit(IS_ROM_STATE_INVALID_ROM_VERSION, &finfo->rom_state)) {
		if (finfo->crc_error == 0 && !other_vendor) {
			...
			if (other_vendor && !(position == SENSOR_POSITION_REAR_TOF ||
				position == SENSOR_POSITION_FRONT_TOF)) {
				if (camfw_full)
					return sprintf(buf, "%s %s %s%d\n", "NULL", "NULL", "NG_FWCDCD", rom_id);
				else
...
static ssize_t camera_mtf_exif_show(char *buf, enum is_cam_info_index cam_index)
{
	struct is_rom_info *finfo;
	struct is_cam_info *cam_info;
	int position;
...
static ssize_t camera_paf_cal_check_show(char *buf, enum is_cam_info_index cam_index)
{
	struct is_rom_info *finfo;
	struct is_cam_info *cam_info;
	int position;
...
/* PAF offset read */
static ssize_t camera_paf_offset_show(char *buf, bool is_mid)
{
	char *cal_buf;
	char tempbuf[10];
	int i;
...
#ifdef CAMERA_EEPROM_SUPPORT_FRONT
...
void ois_power_control(int onoff)
{
	...
	case 0:
		if (!camera_running
			...
			pr_info("%s OIS is already off. Do not power off\n", __func__);
		}
		break;
	case 1:
		if (!camera_running
			...
			pr_info("%s OIS is already on. Do not power on\n", __func__);
		}
		ois_power_count++;
		break;
	default:
...
#endif
		info("%s ois_threshold = %d, result =  %s\n", __func__, ois_threshold, wide_buffer);
...
#ifndef CAMERA_FRONT_FIXED_FOCUS
#ifdef CAMERA_FRONT_PAFCAL
	&dev_attr_front_paf_cal_check.attr,
#endif
	&dev_attr_front_afcal.attr,
#endif
	&dev_attr_front_moduleid.attr,
#endif
#ifdef CAMERA_FRONT_DUAL_CAL
	&dev_attr_front_dualcal.attr,
	&dev_attr_front2_dualcal.attr,
#endif
#ifdef CAMERA_FRONT2
...
#ifdef CAMERA_FRONT2_TILT
	&dev_attr_front2_tilt.attr,
#endif
#ifdef USE_CAMERA_HW_BIG_DATA
	&dev_attr_front2_hwparam.attr,
#endif
#ifdef CAMERA_FRONT2_MODULEID
	&dev_attr_front2_moduleid.attr,
#endif
#endif
#ifdef USE_CAMERA_HW_BIG_DATA
	&dev_attr_front_hwparam.attr,
#endif
...
static struct attribute_group is_sysfs_attr_group_front = {
	.attrs	= is_sysfs_attr_entries_front,
};

static const struct attribute_group *is_sysfs_attr_groups_front[] = {
	&is_sysfs_attr_group_front,
	NULL,
};
...
#ifdef CAMERA_REAR_DUAL_CAL
	&dev_attr_rear_dualcal.attr,
	&dev_attr_rear2_dualcal.attr,
	&dev_attr_rear3_dualcal.attr,
#endif
#ifdef CAMERA_REAR2
...
#ifdef CAMERA_REAR2_AFCAL
	&dev_attr_rear2_afcal.attr,
	&dev_attr_rear2_paf_cal_check.attr,
#endif
#ifdef CAMERA_REAR2_TILT
	&dev_attr_rear2_tilt.attr,
#endif
#ifdef USE_CAMERA_HW_BIG_DATA
	&dev_attr_rear2_hwparam.attr,
#endif
#ifdef CAMERA_REAR2_MODULEID
	&dev_attr_rear2_moduleid.attr,
#endif
#endif

#ifdef CAMERA_REAR3
	&dev_attr_rear3_camtype.attr,
...
#ifdef CAMERA_REAR3_AFCAL
	&dev_attr_rear3_afcal.attr,
	&dev_attr_rear3_paf_cal_check.attr,
#endif
#ifdef CAMERA_REAR3_TILT
	&dev_attr_rear3_tilt.attr,
#endif
#ifdef USE_CAMERA_HW_BIG_DATA
	&dev_attr_rear3_hwparam.attr,
#endif
#ifdef CAMERA_REAR3_MODULEID
	&dev_attr_rear3_moduleid.attr,
#endif
#endif
...
#ifdef CAMERA_REAR4_AFCAL
	&dev_attr_rear4_afcal.attr,
	&dev_attr_rear4_paf_cal_check.attr,
#endif
#ifdef CAMERA_REAR4_TILT
	&dev_attr_rear4_tilt.attr,
#endif
#ifdef USE_CAMERA_HW_BIG_DATA
	&dev_attr_rear4_hwparam.attr,
#endif
#ifdef CAMERA_REAR4_MODULEID
	&dev_attr_rear4_moduleid.attr,
#endif
#endif
...
#ifdef USE_CAMERA_HW_BIG_DATA
	&dev_attr_rear_hwparam.attr,
#endif
	&dev_attr_rear_calcheck.attr,
	&dev_attr_fw_update.attr,
#ifdef FORCE_CAL_LOAD
	&dev_attr_rear_force_cal_load.attr,
#endif
	&dev_attr_rear_camfw_all.attr,
...
static struct attribute_group is_sysfs_attr_group_rear = {
	.attrs	= is_sysfs_attr_entries_rear,
};

static const struct attribute_group *is_sysfs_attr_groups_rear[] = {
	&is_sysfs_attr_group_rear,
	NULL,
};
...
static struct attribute_group is_sysfs_attr_group_ois = {
...
int is_create_sysfs(struct is_core *core)
{
	...
	camera_front_dev = device_create_with_groups(camera_class, NULL, 1, NULL, is_sysfs_attr_groups_front, "front");
	if (IS_ERR(camera_front_dev))
		pr_err("failed to create front device!\n");

	camera_rear_dev = device_create_with_groups(camera_class, NULL, 2, NULL, is_sysfs_attr_groups_rear, "rear");
...
{
	sysfs_remove_group(&camera_front_dev->kobj, &is_sysfs_attr_group_front);
	sysfs_remove_group(&camera_rear_dev->kobj, &is_sysfs_attr_group_rear);
#ifdef CONFIG_OIS_USE
