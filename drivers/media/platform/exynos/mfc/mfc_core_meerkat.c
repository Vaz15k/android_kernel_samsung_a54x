...
static void __mfc_merge_errorinfo_data(struct mfc_core *core, bool px_fault)
{
	...
	idx = __mfc_change_hex_to_ascii(core, core->logging_data->fault_addr, 16, errorinfo, idx);
...
static void __mfc_save_logging_sfr(struct mfc_core *core)
{
	...
	core->logging_data->power_cnt = mfc_core_get_pwr_ref_cnt(core);
	core->logging_data->clock_cnt = mfc_core_get_clk_ref_cnt(core);
...
void __mfc_core_dump_state(struct mfc_core *core, int curr_ctx)
{
	...
	dev_err(core->device, "fw addr %#llx size %#zu drm_fw addr %#llx\n",
...
void __mfc_dump_nal_q_buffer_info(struct mfc_core *core, int curr_ctx)
{
	...
	if (ctx->type == MFCINST_DECODER) {
		dev_err(core->device, "Decoder scratch:%#llx++%#x, static(vp9):%#llx++%#x, MV:++%#lx\n",
				MFC_CORE_DMA_READL(MFC_REG_D_SCRATCH_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_D_SCRATCH_BUFFER_SIZE),
				MFC_CORE_DMA_READL(MFC_REG_D_STATIC_BUFFER_ADDR),
		...
		dev_err(core->device, "Encoder scratch:%#llx++%#x, recon[0]:++%#lx, [1]:++%#lx, ME:++%#lx\n",
				MFC_CORE_DMA_READL(MFC_REG_E_SCRATCH_BUFFER_ADDR),
	...
	for (i = MFC_TRACE_NAL_QUEUE_PRINT - 1; i >= 0; i--) {
		...
		if (pDecIn->StartCode == NAL_Q_DECODER_MARKER)
			dev_err(core->device, "[%d] In DEC[ctx:%d] id %d cpb %#llx used %#x(%d) dpb [0]%#llx [1]%#llx\n",
					cnt, pDecIn->InstanceId, pDecIn->CommandId,
					MFC_NALQ_DMA_READL(pDecIn->CpbBufferAddr),
					pDecIn->DynamicDpbFlagLower,
					ffs(pDecIn->DynamicDpbFlagLower) - 1,
					MFC_NALQ_DMA_READL(pDecIn->FrameAddr[0]),
					MFC_NALQ_DMA_READL(pDecIn->FrameAddr[1]));
		else if (pEncIn->StartCode == NAL_Q_ENCODER_MARKER)
			dev_err(core->device, "[%d] In ENC[ctx:%d] id %d frame [0]%#llx [1]%#llx stream %#llx\n",
					cnt, pEncIn->InstanceId, pEncIn->CommandId,
					MFC_NALQ_DMA_READL(pEncIn->FrameAddr[0]),
					MFC_NALQ_DMA_READL(pEncIn->FrameAddr[1]),
					MFC_NALQ_DMA_READL(pEncIn->StreamBufferAddr));
	}
	...
	for (i = MFC_TRACE_NAL_QUEUE_PRINT - 1; i >= 0; i--) {
		...
		if (pDecOut->StartCode == NAL_Q_DECODER_MARKER)
			dev_err(core->device, "[%d] Out DEC[ctx:%d] id %d disp stat %#x disp [0]%#llx [1]%#llx used %#x dec stat %#x dec [0]%#llx [1]%#llx type %d\n",
					cnt, pDecOut->InstanceId, pDecOut->CommandId,
					pDecOut->DisplayStatus,
					MFC_NALQ_DMA_READL(pDecOut->DisplayAddr[0]),
					MFC_NALQ_DMA_READL(pDecOut->DisplayAddr[1]),
					pDecOut->UsedDpbFlagLower, pDecOut->DecodedStatus,
					MFC_NALQ_DMA_READL(pDecOut->DecodedAddr[0]),
					MFC_NALQ_DMA_READL(pDecOut->DecodedAddr[1]),
			...
			dev_err(core->device, "[%d] Out ENC[ctx:%d] id %d frame [0]%#x [1]%#x stream %#x type %d recon [0]%#x [1]%#x\n",
					...
					MFC_NALQ_DMA_READL(pEncOut->EncodedFrameAddr[0]),
					MFC_NALQ_DMA_READL(pEncOut->EncodedFrameAddr[1]),
					MFC_NALQ_DMA_READL(pEncOut->StreamBufferAddr),
					pEncOut->SliceType,
					MFC_NALQ_DMA_READL(pEncOut->ReconLumaDpbAddr),
					MFC_NALQ_DMA_READL(pEncOut->ReconChromaDpbAddr));
	}
}
...
void __mfc_dump_buffer_info(struct mfc_core *core)
{
	...
	if (ctx->type == MFCINST_DECODER) {
		dev_err(core->device, "Decoder scratch:%#llx++%#x (%#llx~%#llx), static(vp9):%#llx++%#x\n",
				MFC_CORE_DMA_READL(MFC_REG_D_SCRATCH_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_D_SCRATCH_BUFFER_SIZE),
		...
		dev_err(core->device, "CPB:%#llx++%#x, DPB [0]plane:++%#x, [1]plane:++%#x, [2]plane:++%#x\n",
		...
		dev_err(core->device, "Encoder SRC %dplane, [0]:%#llx++%#x, [1]:%#llx++%#x, [2]:%#llx++%#x\n",
				...
				MFC_CORE_DMA_READL(MFC_REG_E_SOURCE_FIRST_ADDR),
				ctx->raw_buf.plane_size[0],
				MFC_CORE_DMA_READL(MFC_REG_E_SOURCE_SECOND_ADDR),
				ctx->raw_buf.plane_size[1],
				MFC_CORE_DMA_READL(MFC_REG_E_SOURCE_THIRD_ADDR),
				ctx->raw_buf.plane_size[2]);
		dev_err(core->device, "DST:%#llx++%#x, scratch:%#llx++%#x (%#llx~%#llx)\n",
				MFC_CORE_DMA_READL(MFC_REG_E_STREAM_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_E_STREAM_BUFFER_SIZE),
				MFC_CORE_DMA_READL(MFC_REG_E_SCRATCH_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_E_SCRATCH_BUFFER_SIZE),
				core_ctx->scratch_buf.daddr,
...
static int __mfc_store_dump_nal_q_buffer_info(struct mfc_core *core, int curr_ctx, char *buf)
{
	...
	if (ctx->type == MFCINST_DECODER) {
		ret = snprintf(buf, 100, "Decoder scratch:%#llx++%#x, static(vp9):%#llx++%#x, MV:++%#lx\n",
				MFC_CORE_DMA_READL(MFC_REG_D_SCRATCH_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_D_SCRATCH_BUFFER_SIZE),
				MFC_CORE_DMA_READL(MFC_REG_D_STATIC_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_D_STATIC_BUFFER_SIZE), ctx->mv_size);
		...
		ret = snprintf((buf + idx), 300,
				"Encoder scratch:%#llx++%#x, recon[0]:++%#lx, [1]:++%#lx, ME:++%#lx\n",
				MFC_CORE_DMA_READL(MFC_REG_E_SCRATCH_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_E_SCRATCH_BUFFER_SIZE),
				ctx->enc_priv->luma_dpb_size,
				ctx->enc_priv->chroma_dpb_size,
...
static int __mfc_store_dump_buffer_info(struct mfc_core *core, char *buf)
{
	...
	ret = snprintf(buf, 100, "\n-----------dumping MFC buffer info (fault at: %#llx)\n",
			core->logging_data->fault_addr);
	idx += ret;
...
static void __mfc_dump_info_and_stop_hw(struct mfc_core *core)
{
	...
	if (IS_MULTI_CORE_DEVICE(dev) && !two_dump && dev->move_ctx_cnt) {
	...
	if (IS_MULTI_CORE_DEVICE(dev) && !two_dump && core->num_inst) {
		if ((core->id == MFC_DEC_DEFAULT_CORE) && dev->core[MFC_SURPLUS_CORE]->num_inst)
			__mfc_dump_regs(dev->core[MFC_SURPLUS_CORE]);
		else if ((core->id == MFC_SURPLUS_CORE) && dev->core[MFC_DEC_DEFAULT_CORE]->num_inst)
			__mfc_dump_regs(dev->core[MFC_DEC_DEFAULT_CORE]);
	}

	/* Broadcast meerkat to film grain */
	if (ctx->plugin_type == MFC_PLUGIN_FILM_GRAIN) {
		dev_err(dev->device, "[PLUGIN] Film grain working (ctx: %d)\n", ctx->num);
		call_dop(dev->plugin, dump_info, dev->plugin);
	}
...
void mfc_core_meerkat_worker(struct work_struct *work)
{
	...
	if (curr_ctx >= 0) {
		...
		if ((core_ctx->state == MFCINST_ERROR) || (core->state == MFCCORE_ERROR)) {
			mfc_info("[MSR] It's Error state: core_ctx %d core %d\n",
