...
/* Used for Flicker sensor self test */
int sm5714_fled_torch_gpio(u8 intensity)
{
	...
	if (intensity == 0) {
		...
		pr_info("sm5714-fled: %s: off mode\n", __func__);
	} else {
		fled_set_mled_current(fled, intensity);
		pr_info("sm5714-fled: %s before on. torch_cnt %d", __func__, fled->torch_on_cnt);
		...
		fled->torch_on_cnt++;
		pr_info("sm5714-fled: %s: gpio control. intensity offset %d \n", __func__, intensity);
	}
	mutex_unlock(&fled->fled_mutex);
	usleep_range(1000, 1000);
	pr_info("sm5714-fled: %s: done (oper mode:0x%x)",__func__, sm5714_charger_oper_get_current_status());
	return ret;
}
EXPORT_SYMBOL_GPL(sm5714_fled_torch_gpio);
