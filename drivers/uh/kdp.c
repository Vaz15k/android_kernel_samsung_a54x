...
#endif
	set_rocred_ucounts((struct cred *)new_ro, get_ucounts(new_ro->cred.ucounts));
	if (!new_ro->cred.ucounts)
	...
	if (is_kdp_protect_addr((u64)cred)) {
		if (cred == &init_cred)
			uh_call(UH_APP_KDP, SET_CRED_UCOUNTS, (u64)cred, (u64)&init_cred,
					(u64)&(init_cred.ucounts), (u64)new_ucounts);
		else
			uh_call(UH_APP_KDP, SET_CRED_UCOUNTS, (u64)cred, (u64)&init_cred,
...
inline void kdp_set_test(unsigned long long  a, unsigned long long b, unsigned long long c)
{
	uh_call(UH_APP_KDP, SET_NS_TEST, (u64)a, (u64)b, (u64)c, 0);
}
