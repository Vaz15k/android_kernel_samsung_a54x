...
|| defined(CONFIG_EXYNOS_ESCA) || defined(CONFIG_EXYNOS_ESCA_MODULE)
				...
				unsigned int *cmd)
{
	...
	bcm_cmd = cmd;
#if defined(CONFIG_EXYNOS_ESCA) || defined(CONFIG_EXYNOS_ESCA_MODULE)
	bcm_cmd[3] |= BCM_CMD_SET(ipc_type, BCM_ONE_BIT_MASK, BCM_EVT_IPCTYPE_SHIFT);
	config.cmd = bcm_cmd;
...
//	memcpy(config.cmd, bcm_cmd, sizeof(unsigned int) * CMD_DATA_MAX);
	...
	ipc_err = exynos_bcm_dbg_ipc_err_handle(config.cmd[0]);
#elif defined(CONFIG_EXYNOS_ADV_TRACER) || defined(CONFIG_EXYNOS_ADV_TRACER_MODULE)
...
#if defined(CONFIG_EXYNOS_ESCA) || defined(CONFIG_EXYNOS_ESCA_MODULE)
/*static int bcm_dbg_handler(struct ipc_config *config, unsigned int len)
{
	return 0;
}*/
#elif defined(CONFIG_EXYNOS_ADV_TRACER) || defined(CONFIG_EXYNOS_ADV_TRACER_MODULE)
static int bcm_dbg_handler(struct adv_tracer_ipc_cmd *cmd, unsigned int len)
{
	return 0;
}
#endif
...
{
	BCM_INFO("INLINE: %s: ipc channel info: ch_num(%u), size(%u)\n",
			__func__, data->ipc_ch_num, data->ipc_size);
...
#if defined(CONFIG_EXYNOS_ESCA) || defined(CONFIG_EXYNOS_ESCA_MODULE)
	ret = esca_ipc_request_channel(data->dev->of_node,
				(ipc_callback)NULL,
				&data->ipc_ch_num, &data->ipc_size);

	BCM_INFO("ESCAESCAESCA%s: ipc channel info: ch_num(%u), size(%u)\n", __func__,
				data->ipc_ch_num, data->ipc_size);
#elif defined(CONFIG_EXYNOS_ADV_TRACER) || defined(CONFIG_EXYNOS_ADV_TRACER_MODULE)
	ret = adv_tracer_ipc_request_channel(data->ipc_node,
				(ipc_callback)bcm_dbg_handler,
				&data->ipc_ch_num, &data->ipc_size);
#endif
	BCM_INFO("%s: ipc channel info: ch_num(%u), size(%u)\n", __func__,
				data->ipc_ch_num, data->ipc_size);

	if (ret) {
		BCM_ERR("%s(%d): request ipc channel is failed\n", __func__, ret);
		return ret;
	}
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->ip_range == BCM_EACH)
		cmd[3] |= BCM_CMD_SET(bcm_ip_index, BCM_IP_MASK, BCM_IP_SHIFT);//problem
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->ip_range == BCM_EACH)
		cmd[3] |= BCM_CMD_SET(bcm_ip_index, BCM_IP_MASK, BCM_IP_SHIFT);
...
#if defined(CONFIG_EXYNOS_BCM_TIME_SYNC)
static int exynos_bcm_dbg_rtc_sync(struct exynos_bcm_ipc_base_info *ipc_base_info,
					u32 rtc_tick,
					struct exynos_bcm_dbg_data *data)
{
	unsigned int cmd[4] = {0, 0, 0, 0};
...
static int exynos_bcm_dbg_ktime_sync(struct exynos_bcm_ipc_base_info *ipc_base_info,
					u64 ktime,
					struct exynos_bcm_dbg_data *data)
{
	unsigned int cmd[4] = {0, 0, 0, 0};
	unsigned int low_ktime, high_ktime;
	int ret = 0;
	unsigned long flags;
	int i = 0;
...
static int exynos_bcm_time_pair_sync(struct exynos_bcm_dbg_data *data)
{
	struct exynos_bcm_ipc_base_info ipc_base_info;
	int ret;
	u64 ktime;
	...
	rtc_tick = __raw_readl(data->rtc_addr);
	mb();
	ktime = ktime_get();
	mb();

	exynos_bcm_dbg_set_base_info(&ipc_base_info, BCM_EVT_KTIME_SYNC,
					BCM_EVT_SET, 0);

	ret = exynos_bcm_dbg_ktime_sync(&ipc_base_info, ktime, data);
	if (ret) {
		BCM_ERR("%s: failed sync ktime\n", __func__);
		return ret;
	}
	exynos_bcm_dbg_set_base_info(&ipc_base_info, BCM_EVT_RTC_SYNC,
					BCM_EVT_SET, 0);

	ret = exynos_bcm_dbg_rtc_sync(&ipc_base_info, rtc_tick, data);
	if (ret) {
		BCM_ERR("%s: failed sync rtc tick\n", __func__);
		return ret;
	}
	return 0;
}
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->direction == BCM_EVT_GET) {
		run = BCM_CMD_GET(cmd[2], BCM_ONE_BIT_MASK,
					BCM_EVT_RUN_CONT_SHIFT);
		*bcm_run = run;
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->direction == BCM_EVT_GET) {
		period = BCM_CMD_GET(cmd[2], BCM_EVT_PERIOD_CONT_MASK,
					BCM_EVT_PERIOD_CONT_SHIFT);
		*bcm_period = period;
	}
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->direction == BCM_EVT_GET) {
		mode = BCM_CMD_GET(cmd[2], BCM_EVT_MODE_CONT_MASK,
					BCM_EVT_MODE_CONT_SHIFT);
		*bcm_mode = mode;
	}
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->direction == BCM_EVT_GET) {
		suspend = BCM_CMD_GET(cmd[2], BCM_ONE_BIT_MASK,
					BCM_EVT_STR_STATE_SHIFT);
		*ap_suspend = suspend;
	}
#if defined(CONFIG_EXYNOS_BCM_DBG_GNR) || defined(CONFIG_EXYNOS_BCM_DBG_GNR_MODULE)
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->direction == BCM_EVT_GET) {
		ip_enable = BCM_CMD_GET(cmd[2], BCM_ONE_BIT_MASK,
					BCM_EVT_IP_CONT_SHIFT);
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (ipc_base_info->direction == BCM_EVT_GET) {
		glb_en = BCM_CMD_GET(cmd[2], BCM_ONE_BIT_MASK,
					BCM_EVT_GLBAUTO_SHIFT);
		*glb_auto_en = glb_en;
	}
...
#if defined(CONFIG_EXYNOS_BCM_TIME_SYNC)
	if(bcm_run == BCM_RUN) {
		ret = exynos_bcm_time_pair_sync(data);
		if (ret)
			return ret;
	}
#endif
...
static ssize_t bcm_dbg_dump_show(struct file *file, char __user *user_buf,
...
static const struct file_operations bcm_dbg_dump_show_fops = {
	.open = simple_open,
	.read = bcm_dbg_dump_show,
...
static int exynos_bcm_dbg_probe(struct platform_device *pdev)
{
	...
	debugfs_root = debugfs_create_dir("bcmdbg", NULL);
	if (debugfs_root) {
		debugfs_create_file("dump_show", 0644, debugfs_root, NULL, &bcm_dbg_dump_show_fops);
	}
