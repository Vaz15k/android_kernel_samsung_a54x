...
struct sec_pm_qos_object sec_pm_qos_array[EXYNOS_PM_QOS_NUM_CLASSES];
#endif /* CONFIG_SEC_PM_QOS_MONITOR */
...
static int exynos_pm_qos_power_init(void)
{
	...
	for (i = 0; i < EXYNOS_PM_QOS_NUM_CLASSES; i++) {
		sec_pm_qos_array[i].name = exynos_pm_qos_array[i]->name;
		sec_pm_qos_array[i].constraints = exynos_pm_qos_array[i]->constraints;
	}
	sec_pm_qos_monitor_init_data(sec_pm_qos_array);
#endif /* CONFIG_SEC_PM_QOS_MONITOR */
