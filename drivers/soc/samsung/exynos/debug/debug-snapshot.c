...
static void dbg_snapshot_kernel_hook_logger(const char *buf, size_t size)
{
	...
	u32 offset = DSS_OFFSET_LAST_LOGBUF;
...
static void dbg_snapshot_first_hook_logger(const char *buf, size_t size)
{
	static DEFINE_SPINLOCK(lock);
	size_t last_buf;
	unsigned long flags;
	void *dest;
	struct dbg_snapshot_item *item = dbg_snapshot_get_item_by_index(DSS_ITEM_FIRST_ID);
	u32 offset = DSS_ITEM_FIRST_ID;
...
static void dbg_snapshot_init_desc(struct device *dev)
{
	...
	if (!of_property_read_u32(dev->of_node, "console_kmsg", &val))
		dss_desc.console_kmsg = val;
...
#define TEMP_KERNEL_LOG_BUF_NUM		(3)
static u64 kmsg_seq;
...
static void dbg_snapshot_init_logbuf(struct printk_ringbuffer *rb)
{
	...
	dbg_snapshot_kernel_hook_logger(text, len);
	dbg_snapshot_first_hook_logger(text, len);
	...
	for(;;) {
		...
		dbg_snapshot_kernel_hook_logger(text, len + 1);
		dbg_snapshot_first_hook_logger(text, len + 1);
	}
}

static void dbg_snapshot_android_vh_logbuf(void *ignore,
...
static void dbg_snapshot_android_vh_logbuf_pr_cont(void *ignore,
						   struct printk_record *r,
						   size_t text_len)
{
	char *text;
	...
	dbg_snapshot_kernel_hook_logger(text, text_len);
	dbg_snapshot_first_hook_logger(text, text_len);
}

static void register_kmsg(struct device *dev)
{
	if (dss_desc.console_kmsg) {
		register_console(&dss_console);
		console_suspend_enabled = false;
	} else {
		register_trace_android_vh_logbuf(dbg_snapshot_android_vh_logbuf, NULL);
		register_trace_android_vh_logbuf_pr_cont(dbg_snapshot_android_vh_logbuf_pr_cont,
							NULL);
	}
}
