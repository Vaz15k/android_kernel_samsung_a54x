...
/*
 * arg : cpu<<4 | lev
 * cpu : test target CPU number (0~7 or 8)
 * lev : cache level 0:DSU, 1:L1, 2:L2
 */
static void simulate_ECC(char *arg)
{
	...
	if (lev > 2) {
		dev_err(exynos_debug_desc.dev, "Level must be 0~2\n");
		return;
	}
...
static void simulate_SHUTDOWN_HANG(char *arg)
{
	pr_emerg("%s called\n", __func__);
	if (exynos_debug_desc.shutdown_hang_enable)
		return;
	mutex_lock(&exynos_debug_desc.shutdown_lock);
...
void edt_shutdown_handler(struct platform_device *pdev)
{
...
static void exynos_debug_test_desc_init(void)
{
	...
	mutex_init(&exynos_debug_desc.shutdown_lock);
