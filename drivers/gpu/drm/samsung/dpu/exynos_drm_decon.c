		...
		const struct exynos_display_mode *exynos_mode)
{
	...
	config->hblank = mode->htotal - mode->hdisplay;
	config->vblank = mode->vtotal - mode->vdisplay;
#if IS_ENABLED(CONFIG_DRM_MCD_COMMON)
	...
	if (exynos_mode->dsc.enabled) {
		...
		memcpy(&config->pps, &exynos_mode->pps, sizeof(struct exynos_drm_pps));
	}
				...
				struct exynos_drm_crtc_state *new_exynos_crtc_state)
{
	...
	for_each_new_connector_in_state(crtc_state->state, conn, new_conn_state, i) {
		...
		if (conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
			...
			new_exynos_conn_state =
				to_exynos_connector_state(new_conn_state);

			if (!need_panel_recovery(exynos_crtc, new_exynos_conn_state) &&
					(is_tui_trans(crtc_state) ||
					new_exynos_conn_state->is_lp_transition))
			      ...
			      struct drm_atomic_state *state)
{
	...
	decon_check_display_config(exynos_crtc, new_exynos_crtc_state);
			...
			struct exynos_drm_crtc_state *exynos_crtc_state)
{
	...
	if (decon_reg_wait_update_done_timeout(decon->id,
		...
		if (atomic_inc_return(&exynos_crtc->d.shadow_timeout_cnt) >= TIMEOUT_CNT) {
			if (__is_recovery_supported(decon)) {
				if (decon_trigger_recovery(exynos_crtc, "dsim|peri") < 0) {
...
/* Enable svsync log for temp */
static bool dpu_svsync_log = 1;
...
static void _decon_enable(struct decon_device *decon)
{
	...
	exynos_rcd_enable(decon->crtc->rcd);
#endif
...
#if IS_ENABLED(CONFIG_EXYNOS_DMA_RCD)
	exynos_rcd_disable(decon->crtc->rcd);
#endif
...
static void decon_parse_urgent_info(struct decon_device *decon, struct device_node *np)
{
	...
	urgent->dta_en = of_property_read_bool(np, "dta_en");
	if (urgent->dta_en) {
...
static int decon_parse_bts_info(struct decon_device *decon, struct device_node *np)
{
	...
	for (i = 0; i < bts->dfs_lv_cnt; i++)
		decon_info(decon, KERN_CONT "%6d ", bts->dfs_lv[i]);
