...
static int parse_hdr_info(struct device_node *node, struct panel_hdr_info *hdr)
{
	...
	ret = of_property_read_u32(node, "SLSI,hdr_num", &hdr_num);
	if ((ret) || (hdr_num == 0)) {
	...
	ret = of_property_read_u32_array(node, "SLSI,hdr_type", hdr_type, hdr_num);
	if (ret) {
...
u32 mcd_drm_panel_get_reset_pos(struct exynos_panel *ctx)
{
	...
	ret = of_property_read_u32(np, "SLSI,reset_pos", &reg);
