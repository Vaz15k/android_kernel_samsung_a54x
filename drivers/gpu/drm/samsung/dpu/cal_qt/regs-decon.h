 ...
 * - CMU_DPUF0               0x???
 * - D_TZPC_DPU0             0x????
 * - SYSREG_DPU0             0x1482_0000 : DRCG (0x0104)
...
#define DECON_VERSION_INFO			(0x06200000)
#define DECON_VERSION_GET(_v)			(((_v) >> 0) & 0xffffffff)
#define DECON_VERSION_GET_MAJOR(_v)		(((_v) >> 24) & 0x000000ff)
#define DECON_VERSION_GET_MINOR(_v)		(((_v) >> 16) & 0x000000ff)
...
#define SHD_REG_UP_REQ_DQE_ROI			(1 << 29) // decon0 only
#define SHD_REG_UP_REQ_DQE			(1 << 28) // decon0 only
...
#define INT_PEND_RESOURCE_CONFLICT		(1 << 4) // Was not there in ML4
#define INT_PEND_TIME_OUT			(1 << 0)

/* defined for common part of driver only */
// Was not there in ML4
...
/* Only SRAM0 & SRAM1 are applicable for Quartz */
...
/* SRAM enable/disable sharing for primary display path */
...
#define DITHER_CON				(0x0250) // Only in DECON1
#define DITH_MASK_SEL				(1 < 1)
#define DITH_MASK_PIN				(1 < 0)
...
#define CLOCK_CON_AUTO_CG_MASK                 (0x11111 << 0) // bit [20] is reserved, not set
...
// Considering [31:29] ie. [7:5] of MSB as reserved bits, mask is 0x1f
#define PPS36_FLATNESS_MIN_QP_MASK		(0x1f << 24)
#define PPS36_FLATNESS_MIN_QP(_v)		((_v) << 24)
// Considering [23:21] ie. [7:5] of MSB as reserved bits, mask is 0x1f
...
// Considering [31:28] ie. [7:4] of MSB as reserved bits, mask is 0xf
...
// Considering [15:13] ie. [7:5] of MSB as reserved bits, mask is 0x1f
