...
static void decon_reg_set_outfifo_size_ctl2(u32 id, u32 width, u32 height)
{
	...
	/* In Quartz, OF_SIZE_2 is available only in decon0 */
	if (id != 0)
		return;
...
static void decon_reg_set_data_path(u32 id, struct decon_config *cfg)
{
	...
	switch (out_type) {
	case DECON_OUT_DSI0:
		/* s5e8835 has DECON0/OUT0 connected only to DSIMIF0 */
		val = OUTIF_DSI0;
...
/* Quartz S5E8835,
 * Only SWB (Standalone WriteBack is supported
 * CWB (Cocurrent WriteBack is not supported
 */
...
void decon_reg_set_rcd_enable(u32 id, bool en)
{
	...
	val = en ? ENHANCE_DQE_ON_RCD_ON : 0;
	decon_write_mask(id, DATA_PATH_CON, val, ENHANCE_DQE_ON_RCD_ON);
}
#endif
...
static u32 decon_reg_get_data_path_cfg(u32 id, enum decon_path_cfg con_id)
{
	...
	switch (con_id) {
	/* Quartz supports COMP0/DSC0 */
	case PATH_CON_ID_DSC0:
		if (d_path & (0x1 << PATH_CON_ID_DSC0))
			bRet = 1;
		break;
	case PATH_CON_ID_WB:
	...
	case PATH_CON_ID_DSIM_IF0:
		...
		break;
	default:
		cal_log_warn(id, "Invalid COMP_OUTIF_PATH set in DATA_PATH_CON reg\n");
		break;
	}
		...
		u32 bpc)
{
	...
	if (dsc->enabled) {
		...
		comp_slice_width = get_comp_dsc_width(dsc, bpc);
		if (dsc->dsc_count == 1) {
			...
			cal_log_warn(id, "dsc_count == 2 is invalid\n");
		}
	} else {
...
static void decon_reg_config_win_channel(u32 id, u32 win_idx, int ch)
{
	...
	/* TBD: Win 0~3, channels 0~3
	 * Win 4~7, Channel 0~15
	 */
	val = WIN_CHMAP_F(ch);
	mask = WIN_CHMAP_MASK;
...
static u32 decon_get_ewr_cycle(int fps, int wakeup_us)
{
	...
	cnt = ((1000000 / fps) - wakeup_us) * 52; //OSC Clk Rate
		...
		bool in_slice[4], u32 ds_en[2], u32 sm_ch[2])
{
	...
	case 4:
		...
		/* ENC1 is not supported in S5E8835 */
				...
				struct decon_dsc *dsc_enc)
{
	...
	u32 dsc_enc0_w = 0;
	u32 dual_slice_en = 0;
	struct exynos_drm_pps *pps = &config->pps;

	/* 3bytes per pixel */
	slice_width_byte_unit = pps->slice_width * 3;
	...
	dsc_enc->comp_cfg = exynos_drm_dsc_get_comp_config(pps);
	dsc_enc->bit_per_pixel = pps->bits_per_pixel;
	dsc_enc->pic_height = pps->pic_height;
	dsc_enc->pic_width = pps->pic_width;
	dsc_enc->slice_height = pps->slice_height;
	dsc_enc->slice_width = pps->slice_width;
	dsc_enc->chunk_size = pps->chunk_size;
	dsc_enc->initial_xmit_delay = pps->initial_xmit_delay;
	dsc_enc->initial_dec_delay = pps->initial_dec_delay;
	dsc_enc->initial_scale_value = pps->initial_scale_value;
	dsc_enc->scale_increment_interval = pps->scale_increment_interval;
	dsc_enc->scale_decrement_interval = pps->scale_decrement_interval;
	dsc_enc->first_line_bpg_offset = pps->first_line_bpg_offset;
	dsc_enc->nfl_bpg_offset = pps->nfl_bpg_offset;
	dsc_enc->slice_bpg_offset = pps->slice_bpg_offset;
	dsc_enc->initial_offset = pps->initial_offset;
	dsc_enc->final_offset = pps->final_offset;
	dsc_enc->rc_range_parameters = exynos_drm_dsc_get_rc_range_params(pps, 0);
			...
			struct decon_dsc *dsc_enc, bool gen_pps)
{
	...
	/* gen_pps is made false in S5E8835 */
	if (gen_pps)
	...
	if (id == 1) {
		...
		cal_log_debug(id, "Decon%d & DP not supported\n", id);
	} else {
...
static void decon_reg_update_req_global(u32 id)
{
	...
	if (decon_read_mask(id, DATA_PATH_CON, ENHANCE_PATH_MASK))
		...
		struct decon_window_regs *regs)
{
	...
	/* TBD: DECON_BLENDING_NONE etc are not matching with Olympus
	 * fbdev enum values... Will there be issue with HWC ??
	 */
...
#if 0 // move disp dither settings into dqe driver as DQE can be disabled
	/* TODO decon1 also use dqe but not considered */
	if (id == 0) {
...
int decon_reg_instant_stop(u32 id, struct decon_config *config, bool rst, u32 fps)
{
	...
	/* call instant off directly without perframe stop */
	if (config->out_type & DECON_OUT_DSI) {
...
static void decon_reg_read_resource_status(u32 id, u32 opt)
{
	...
	cal_log_warn(id, "decon%d RSC_STATUS_7: SEL_DSCC = 0x%x\n",
		id, decon_read(id, RSC_STATUS_7));
	if (opt) {
		for (i = 0; i < 5; i++) {
...
int decon_reg_get_interrupt_and_clear(u32 id, u32 *ext_irq)
{
	...
	/* TBD: Add other interrupts as well
	 */
...
void sramc_d_reg_get_irq_and_clear(u32 id) { return; }
