...
struct cphy_timing_value {
	unsigned int sps;
	unsigned int trio_prepare;
	unsigned int trio_prebegin;
	unsigned int trio_post;
	unsigned int lpx;
	unsigned int hs_exit;
	unsigned int trio_settle;
	unsigned int b_cphyctl;
};
...
#define DSIM_CAL_CLK_DIVIDER_CPHY	7
#define DSIM_CAL_HS_THRESHOLD_CPHY	500

#define DSIM_CAL_LINE_CNT_DPHY		4
#define DSIM_CAL_CLK_DIVIDER_DPHY	16
#define DSIM_CAL_HS_THRESHOLD_DPHY	1500

struct cal_phy_priv {
	enum dsi_intf_type 	type;
	int			line_cnt;
	u32			div;
	u32			hs_thres;
};
