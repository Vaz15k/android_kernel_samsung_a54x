	...
	if (usb->user_scenario !=
		...
		if (new_mode == AUDIO_MODE_IN_CALL &&
			...
			pr_info("resume usage = %x, active = %d, err = %d\n",
				usb_audio.chip->intf[0]->dev.power.usage_count,
...
void exynos_usb_audio_disconn(struct usb_interface *intf)
{
	...
	send_usb_vendor_notify_audio_uevent(udev, chip->card->number, 0);
	send_usb_vendor_notify_cardnum(chip->card->number, 0, 0);
#endif
#endif
			...
			enum snd_vendor_pcm_open_close onoff, int direction)
{
	...
	send_usb_vendor_notify_pcm_info(direction, enable);
#endif
	if (onoff == 1) {
...
#ifdef CONFIG_USB_AUDIO_ENHANCED_DETECT_TIME
#if IS_ENABLED(CONFIG_USB_VENDOR_NOTIFY)
		send_usb_vendor_notify_cardnum(chip->card->number, 0, 1);
		send_usb_vendor_notify_audio_uevent(chip->dev, chip->card->number, 1);
#endif
#endif
		return 0;
	}
